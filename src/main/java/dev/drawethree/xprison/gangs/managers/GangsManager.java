...
public class GangsManager {
    ...
    public void invitePlayer(Player invitedBy, Player invited) {
        Schedulers.async().run(() -> {
            ...
            getMaxGangMembers(gang).whenComplete((max, ex) -> {
                if (ex != null) {
                    ex.printStackTrace();
                    invitedBy.sendMessage(ChatColor.RED + "Something went wrong, please contact an admin.");
                    return;
                }

                if (gang.getMembersOffline().size() >= max) {
    ...
    private CompletableFuture<Integer> getMaxGangMembers(Gang gang) {
        return CompletableFuture.supplyAsync(() -> {
            int limit = this.plugin.getConfig().getMaxGangMembers();

            for (UUID gangMember : gang.getGangMembers()) {
                User user = LuckPermsProvider.get().getUserManager().getUser(gangMember);

                if (user == null)
                    user = LuckPermsProvider.get().getUserManager().loadUser(gangMember).join();

                final int userLimit = user.getCachedData()
                        .getPermissionData()
                        .getPermissionMap()
                        .entrySet().
                        stream()
                        ...
                        .filter(node -> node.startsWith("prison.gang.members."))
                        .mapToInt(node -> {
                            final String lastBit = node.substring(node.length() + 1);

                            return Objects.requireNonNullElse(Ints.tryParse(lastBit), -1);
                        })
                        .max()
                        .orElse(-1);
    ...
    private List<String> getGangInfoFormat(Gang g) {
        ...
        for (String s : originalFormat) {
            returnList.add(s.replace("%gang_top%", String.format("%,d", this.getGangTopPosition(g))).replace("%gang_value%", String.format("%,d", g.getValue())).replace("%gang_money%", NumberFormatter.formatNumber(g.getMoney())).replace("%gang%", TextUtils.applyColor(g.getName())).replace("%gang_owner%", g.getOwnerOffline().getName()).replace("%gang_members%", StringUtils.join(g.getMembersOffline().stream().map(OfflinePlayer::getName).toArray(), ", ")));
        }
        return returnList;
    }
            ...
            if (s.startsWith("{FOR_EACH_GANG}")) {
                ...
                for (int i = 0; i < 10; i++) {
                    try {
                        ...
                        PlayerUtils.sendMessage(sender, rawContent.replace("%position%", String.valueOf(i + 1)).replace("%gang%", gang.getName()).replace("%value%", String.format("%,d", gang.getValue())).replace("%money%", NumberFormatter.formatNumber(gang.getMoney())));
                    } catch (Exception e) {
                        break;
                    }
                }
